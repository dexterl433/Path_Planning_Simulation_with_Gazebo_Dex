...
for obs=1:NumObstacles
    ...
    for i=iMn:iMx; for j=jMn:jMx
        % Check if ANY part of the cell overlaps the obstacle rectangle
        % Cell spans from CellsX(i)-MeshSize/2 to CellsX(i)+MeshSize/2
        cellLeft  = CellsX(i) - MeshSize/2;
        cellRight = CellsX(i) + MeshSize/2;
        cellBot   = CellsY(j) - MeshSize/2;
        cellTop   = CellsY(j) + MeshSize/2;
        % Two rectangles overlap if they overlap on BOTH axes
        if cellRight>=xMn && cellLeft<=xMx && cellTop>=yMn && cellBot<=yMx
            ObstacleMap(i,j)=1;
        end
    end; end
end
% Inflate obstacles by 1 cell for safety margin
...
% Uses Bresenham-style traversal to check EVERY cell the line touches
    function clear=hasLineOfSight(p1,p2,obsMap,nX,nY)
        ...
        % Bresenham line: visits every cell the line passes through
        err=dx-dy;
        x=x0; y=y0;
        while true
            % Check current cell
            if x>=1&&x<=nX&&y>=1&&y<=nY
                if obsMap(x,y)==1; clear=false; return; end
            else
                clear=false; return;  % out of bounds
            end
            ...
            % Move diagonally — check BOTH adjacent cells to prevent
            % corner cutting between two diagonal obstacles
            if e2>-dy && e2<dx
                % Diagonal step — check both cells we pass between
                if x+sx>=1&&x+sx<=nX&&y>=1&&y<=nY
                    if obsMap(x+sx,y)==1; clear=false; return; end
                end
                if x>=1&&x<=nX&&y+sy>=1&&y+sy<=nY
                    if obsMap(x,y+sy)==1; clear=false; return; end
                end
                err=err-dy+dx;
                x=x+sx; y=y+sy;
            elseif e2>-dy
