...
set(gca,'XTickLabel',names,'FontSize',7); ylabel('Meters'); title('Path Length (Euclidean)'); grid on;
hold on; yline(BestDist,'r--','Straight-line','FontSize',8); hold off;

subplot(2,3,3);
bar([A1Eff,A2Eff,A3Eff,A4Eff,A5Eff,A6Eff]);
set(gca,'XTickLabel',names,'FontSize',7); ylabel('%'); title('Path Efficiency'); grid on;
ylim([0 100]);
...
bar([A1Dist/max(1,A1Steps),A2Dist/max(1,A2Steps),A3Dist/max(1,A3Steps),...
    A4Dist/max(1,A4Steps),A5Dist/max(1,A5Steps),A6Dist/max(1,A6Steps)]);
set(gca,'XTickLabel',names,'FontSize',7); ylabel('m/step'); title('Distance per Step'); grid on;
