...
% Returns true if the segment (x1,y1)→(x2,y2) overlaps the rectangle.
    function hit=segHitsRect(x1,y1,x2,y2,rxMn,ryMn,rxMx,ryMx)
        ...
        % CRITICAL FIX: must check t0 <= t1 after all edges processed
        % Without this, certain diagonal angles can slip through
        hit = (t0 <= t1);
    end
...
for wp=1:size(Waypoints,2)-1
    ...
    for it=1:MaxIter
        ...
        nC=max(abs(newN-nN))*2; ok=true;  % *2 for double resolution
        if nC>0; for c=0:nC
        ...
        if ok
            ...
            if norm(newN-gC)<=StepSz
                nC2=max(abs(gC-newN))*2; gF=true;  % *2 for double resolution
...
for wp=1:size(Waypoints,2)-1
    ...
    % --- 3. Grid-based safety verification ----------------------------
    % Double-check every smoothed segment against InflatedMap.
    % If any segment crosses an inflated cell, fall back to raw A* path.
    verified=true;
    for si=1:size(smoothed,2)-1
        sp1=smoothed(:,si); sp2=smoothed(:,si+1);
        nChk=max(abs(sp2-sp1))*2;  % oversample for safety
        if nChk>0
            for c=0:nChk
                cp=round(sp1+(sp2-sp1)*c/nChk);
        ...
        if ~verified; break; end
    end

    if verified
        A4Path=[A4Path,smoothed];
    else
        disp(['   Note: smoothed path seg ',num2str(wp),' failed verification, using raw A*']);
        A4Path=[A4Path,rawSeg];
    end
end
