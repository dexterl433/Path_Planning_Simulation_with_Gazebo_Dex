...
for alg=1:numAlgs
    ...
    % For Theta* panel: show the raw A* grid path in grey as background
    if alg==4 && ~isempty(A4RawPath)
        plot(CellsX(A4RawPath(1,:)),CellsY(A4RawPath(2,:)),'-','Color',[0.8 0.8 0.8],'LineWidth',1);
    end
end
drawnow;
...
for alg=1:numAlgs
    pth=algPaths{alg}; if isempty(pth); continue; end
    subplot(1,4,alg);
    % For Theta*: draw raw A* path in grey underneath first
    if alg==4 && ~isempty(A4RawPath)
        plot(CellsX(A4RawPath(1,:)),CellsY(A4RawPath(2,:)),'-','Color',[0.75 0.75 0.75],'LineWidth',1);
    end
